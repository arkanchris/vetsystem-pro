...
const getMedicamentoById = async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM medicamentos WHERE id=$1', [req.params.id]
    );
...
// ── CREAR medicamento ─────────────────────────────────────────────────────────
// BUG CORREGIDO: antes faltaba pasar clienteId en los valores del INSERT
const createMedicamento = async (req, res) => {
  try {
    const clienteId = await getClienteId(req.usuario);
    const {
      nombre, descripcion, categoria, unidad,
      stock, stock_minimo, precio_compra, precio_venta, precio,
      fecha_vencimiento
    } = req.body;
    ...
    try {
      result = await pool.query(`
        INSERT INTO medicamentos
          (nombre, descripcion, categoria, unidad, stock, stock_minimo,
           precio, precio_compra, precio_venta, fecha_vencimiento, cliente_id)
      ...
      `, [
        ...
        clienteId   // ← bug corregido: antes faltaba este valor
      ]);
    } catch (e) {
      // Fallback sin cliente_id
      console.warn('Medicamento sin cliente_id:', e.message);
      result = await pool.query(`
        INSERT INTO medicamentos
          (nombre, descripcion, categoria, unidad, stock, stock_minimo,
           precio, precio_compra, precio_venta, fecha_vencimiento)
    ...
    console.error('createMedicamento error:', error);
    res.status(500).json({ error: error.message });
  }
};
  ...
  try {
    ...
    const result = await pool.query(`
      UPDATE medicamentos SET
        nombre=$1, descripcion=$2, categoria=$3, unidad=$4,
        stock=$5, stock_minimo=$6, precio=$7,
        precio_compra=$8, precio_venta=$9, fecha_vencimiento=$10
    ...
    console.error('updateMedicamento error:', error);
    res.status(500).json({ error: error.message });
  }
};
...
const getStockBajo = async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM medicamentos WHERE activo=true AND stock <= stock_minimo ORDER BY stock ASC'
    );
    res.json(result.rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
module.exports = {
  ...
  createMedicamento, updateMedicamento,
  deleteMedicamento, getStockBajo
};