...
const login = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT u.*, c.nombre as cliente_nombre, c.logo_url as cliente_logo
       FROM usuarios u
       LEFT JOIN clientes c ON u.cliente_id = c.id
       WHERE (u.username=$1 OR u.email=$1) AND u.activo=true LIMIT 1`,
    ...
    const token = jwt.sign(
      { id: usuario.id, email: usuario.email, rol: usuario.rol, cliente_id: usuario.cliente_id||null, clinica_nombre: usuario.cliente_nombre || usuario.clinica_nombre || null },
    ...
    } else if (['admin','admin_veterinario'].includes(usuario.rol)) {
    ...
    res.json({ mensaje: '✅ Login exitoso', token, usuario: { id: usuario.id, nombre: usuario.nombre, email: usuario.email, username: usuario.username, rol: usuario.rol, puede_ver_finanzas: usuario.puede_ver_finanzas, clinica_nombre: usuario.cliente_nombre || usuario.clinica_nombre, cliente_id: usuario.cliente_id, modulos: modulosActivos } });
...
const registro = async (req, res) => {
  ...
  try {
    ...
    if (['admin','admin_veterinario'].includes(rol) && req.usuario?.rol !== 'master') return res.status(403).json({ error: '❌ Solo el Máster puede crear admins.' });
